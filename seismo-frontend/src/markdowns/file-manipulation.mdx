...
- `os.path.split(path)`, returns a tuple with the *basename* and the *dirname* of the given path in a Python `tuple` (dirname, basename)
...
To continue, suppose that we move the file inside a folder called *auth* that is located at the current working directory, with a new name of *login.html* and we want to get the new file path: 
...
Lastly, let's say that we need to rename the *login.html* file that is located in the *auth* folder, to *login-template.html*.
