(() => {
  ...
  let detrendTypeSelectExpanding = document.querySelector(
    "#detrend-type-select-expanding"
  );
  let detrendOrderInput = document.querySelector("#detrend-order-input");
  let detrendOrderInputExpanding = document.querySelector(
    "#detrend-order-input-expanding"
  );
  let detrendApplyButton = document.querySelector("#detrend-apply-button");
  let detrendApplyButtonExpanding = document.querySelector(
    "#detrend-apply-button-expanding"
  );
  let taperTypeSelect = document.querySelector("#taper-type-select");
  let taperTypeSelectExpanding = document.querySelector(
    "#taper-type-select-expanding"
  );
  let taperSideSelect = document.querySelector("#taper-side-select");
  let taperSideSelectExpanding = document.querySelector(
    "#taper-side-select-expanding"
  );
  let taperLengthInput = document.querySelector("#taper-length-input");
  let taperLengthInputExpanding = document.querySelector(
    "#taper-length-input-expanding"
  );
  let taperApplyButton = document.querySelector("#taper-apply-button");
  let taperApplyButtonExpanding = document.querySelector(
    "#taper-apply-button-expanding"
  );
  let trimLeftSideInput = document.querySelector("#trim-left-side-input");
  let trimLeftSideInputExpanding = document.querySelector(
    "#trim-left-side-input-expanding"
  );
  let trimRightSideInput = document.querySelector("#trim-right-side-input");
  let trimRightSideInputExpanding = document.querySelector(
    "#trim-right-side-input-expanding"
  );
  let trimApplyButton = document.querySelector("#trim-apply-button");
  let trimApplyButtonExpanding = document.querySelector(
    "#trim-apply-button-expanding"
  );

  let saveFileButton = document.querySelector("#save-file-button");

  let processingPillsContainer = document.querySelector(
    "#processing-pills-container"
  );
  ...
  let config = {
    ...
    displayModeBar: true,
    modeBarButtons: [
      ["pan2d", "zoom2d", "resetScale2d", "resetViews", "toggleSpikelines"],
    ],
  };
  let layout = {
    title: "",
  ...
  let disableEnableElements = [
    detrendTypeSelect,
    detrendTypeSelectExpanding,
    detrendOrderInput,
    detrendOrderInputExpanding,
    detrendApplyButton,
    detrendApplyButtonExpanding,
    taperTypeSelect,
    taperTypeSelectExpanding,
    taperSideSelect,
    taperSideSelectExpanding,
    taperLengthInput,
    taperLengthInputExpanding,
    taperApplyButton,
    taperApplyButtonExpanding,
    trimLeftSideInput,
    trimLeftSideInputExpanding,
    trimRightSideInput,
    trimRightSideInputExpanding,
    trimApplyButton,
    trimApplyButtonExpanding,
  ];
  ...
  detrendTypeSelect.addEventListener("change", () => {
    if (
      detrendTypeSelect.value === "polynomial" ||
      detrendTypeSelect.value === "spline"
    ) {
  ...
  detrendTypeSelectExpanding.addEventListener("change", () => {
    if (
      detrendTypeSelectExpanding.value === "polynomial" ||
      detrendTypeSelectExpanding.value === "spline"
    ) {
      detrendOrderInput.disabled = false;
      detrendOrderInputExpanding.disabled = false;
    } else {
    ...
    fetch("/signal-processing/upload-seismic-file", {
      ...
      .then((mseedData) => {
        ...
        document.querySelector(
          "#signal-processing-start-by-upload-container"
        ).style.display = "none";
        ...
        document.querySelector("#record-name-paragraph").textContent =
          mseedData["trace-0"]["record-name"];
  ...
  function deletePillFilter(elementToDelete, pillButton) {
    ...
    fetch(
      `/signal-processing/delete-applied-filter?filter=${allActiveFiltersString}`
    )
      .then((response) => {
        // if not ok deactivate the spinner and show the modal message
        if (!response.ok) {
