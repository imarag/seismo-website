    ...
    if filter_value != "initial":
        ...
        if freqmin:
            ...
            if freqmin < 0.01 or freqmin > 99:
                error_message = (
                    "The acceptable left filter range is from 0.01 to 99 Hz!"
                )
                return raise_error(error_message)
        ...
        if freqmax:
            ...
            if freqmax < 0.02 or freqmax > 100:
                error_message = (
                    "The acceptable right filter range is from 0.02 to 100 Hz!"
                )
                return raise_error(error_message)
        ...
        if freqmin and freqmax and freqmin >= freqmax:
            error_message = (
                "The left filter cannot be greater or equal to the right filter!"
            )
            return raise_error(error_message)
...
def save_arrivals():
    ...
    if Parr == "null" and Sarr == "null":
        error_message = (
            "You need to select at least one wave arrival to use this option!"
        )
        return raise_error(error_message)
    ...
    arrivals_file_name = (
        str(starttime.date) + "_" + str(starttime.time) + "_" + station + ".txt"
    )
