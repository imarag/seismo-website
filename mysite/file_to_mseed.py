...
def upload_file():
    ...
    if format == "excel":
        try:
            df = pd.read_excel(uploaded_file, header=header_param)
        except Exception as e:
            error_message = str(e)
            return raise_error(error_message)
    elif format == "csv":
        try:
            df = pd.read_csv(uploaded_file, header=header_param)
        except Exception as e:
            error_message = str(e)
            return raise_error(error_message)
    else:
        try:
            df = pd.read_csv(
                uploaded_file, header=header_param, skiprows=skiprows_param, sep=delim
            )
        except Exception as e:
            error_message = str(e)
            return raise_error(error_message)
...
def convert_file_to_mseed():
    ...
    if is_two_components == "true" and (
        vert_compo_column == "select column" or hor_compo1_column == "select column"
    ):
        error_message = (
            "You need to select a column of your file for each of the two components!"
        )
        return raise_error(error_message)
    elif is_two_components == "false" and (
        vert_compo_column == "select column"
        or hor_compo1_column == "select column"
        or hor_compo2_column == "select column"
    ):
        error_message = (
            "You need to select a column of your file for each of the three components!"
        )
        return raise_error(error_message)

    if (
        is_two_components == "true"
        and len(set([vert_compo_column, hor_compo1_column])) != 2
    ):
        error_message = "You cannot have the same column for two components!"
        return raise_error(error_message)
    elif (
        is_two_components == "false"
        and len(set([vert_compo_column, hor_compo1_column, hor_compo2_column])) != 3
    ):
        error_message = "You cannot have the same column for two components!"
        return raise_error(error_message)
    ...
    if not parameterValue:
        error_message = (
            "You need to insert a valid value for the sampling parameters (fs or dt)!"
        )
        return raise_error(error_message)
    else:
        try:
    ...
    for compo in df.columns:
        ...
        trace = Trace(data=trace_data, header=params_dict)
        lt_traces.append(trace)

    st = Stream(lt_traces)
    mseed_file_save_path = create_path("file-to-mseed.mseed")
    st.write(mseed_file_save_path)

    return send_file(
        mseed_file_save_path, as_attachment=True, download_name="mseed-stream.mseed"
    )
