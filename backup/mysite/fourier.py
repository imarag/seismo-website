...
def compute_fourier():
    ...
    if float(signal_window_left_side) + float(window_length) > total_duration:
        ...
        noise_selected == "true"
        and float(noise_window_right_side) - float(window_length) < 0
    ):
    ...
    for n, tr in enumerate(fourier_data_dict):
        ...
        for s_n in fourier_data_dict[tr]:
            ...
            axfourier[n].plot(
                xdata, ydata, color=colors[color_i], lw=1, label=label_name
            )
            color_i += 1
        ...
        axfourier[n].legend(
            ...
            fontsize=11,
            facecolor="w",
            title=get_record_name(mseed_file_path),
            title_fontsize=11,
        )
...
def download():
    ...
    return send_file(file_to_download, as_attachment=True, download_name=name)
